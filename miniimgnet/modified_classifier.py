...
def get_corrected_rel(relations, one_hot_labels): # query_M, support_K * support_N # unit test
    relations = relations.detach()
    one_hot_labels = one_hot_labels.detach()
    corrected_relations = torch.clone(relations)
    for query_i in range(relations.shape(0)):
        scores = relations[query_i]
        rightScoreIs = torch.argwhere(one_hot_labels[query_i] == 1)
        rightScores = scores[rightScoreIs]
        lowestRightScore = torch.min(rightScores)
        for i, score in enumerate(scores):
            if score > lowestRightScore and one_hot_labels[query_i, i] == 0:
                corrected_relations[query_i, i] = 0
        corrected_relations[query_i, rightScoreIs] = 1

...
def main():
    ...
    teacher.load_state_dict(torch.load('./vgg_teacher.pth'))
    teacher.to(device)

    optimizer = torch.optim.SGD(teacher.parameters(), lr = 1e-5, momentum=0.9, weight_decay=5e-4)
    ...
    for i in range(EPOCH):
        ...
        if cur_acc > best_acc:
            torch.save(teacher.state_dict(), './vgg_teacher.pth')

...
def main():
    ...
    for episode in range(EPISODE):
        ...
        batches = batches.repeat(1, 3, 1, 1)

        # calculate features
        sample_features = feature_encoder(Variable(samples).cuda(GPU)) # 5x64*5*5 -> N x 512 * 49
        sample_features = sample_features.view(CLASS_NUM,FEATURE_DIM,49)
        ...
        relation_pairs = torch.cat((sample_features_ext,batch_features_ext),1).view(-1,FEATURE_DIM*2,49)
        ...
        if (episode+1)%5000 == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                # to form a 100x128 matrix for relation network
                sample_features_ext = sample_features.unsqueeze(0).repeat(BATCH_NUM_PER_CLASS*CLASS_NUM,1,1)
                test_features_ext = test_features.unsqueeze(0).repeat(CLASS_NUM,1,1)
                test_features_ext = torch.transpose(batch_features_ext,0,1)

                relation_pairs = torch.cat((test_features_ext,batch_features_ext),1).view(-1,FEATURE_DIM*2,49)
